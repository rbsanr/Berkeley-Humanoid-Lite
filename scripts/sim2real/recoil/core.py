...
class Bus:
    ...
    def __init__(self, channel: str, bitrate: int = 1000000):
        ...
        self.__bus = can.Bus(interface="socketcan", channel=self.channel, bitrate=self.bitrate)
    ...
    def set_mode(self, device_id: int, mode: int) -> None:
    ...
    def write_read_pdo_2(self, device_id: int, position_target: float, velocity_target: float, timeout=0.001) -> tuple:
        self.transmit_pdo_2(device_id, position_target, velocity_target)
        return self.receive_pdo_2(device_id, timeout)
    ...
    def receive_pdo_2(self, device_id: int, timeout=0.001) -> tuple:
        rx_frame = self.receive(filter_device_id=device_id, timeout=timeout)
